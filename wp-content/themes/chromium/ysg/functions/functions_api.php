...
/**
 * REST API requests go through this file
 */
//////////////////////////////////////////////////////
////////////////////////////
//////////////////////////////////////////////////////
...
add_filter(
    ...
    function ($response, $object, $request) {
        ...
        $order_discount  = ysgGetOrderDiscountUsed($order_data);
        ...
        */
        ...
        foreach ($order_data['line_items'] as $key => $item) {
            ...
            $discount_type = ysgSetDiscountType($item, $points, $coupon, $order_discount, $totals);
        ...
        if (isset($order_data['shipping_lines']) && !empty($order_data['shipping_lines'])) {
            foreach ($order_data['shipping_lines'] as $key => $item) {
                if ($item['method_id'] == "ysg_skynet") {
                    $shipping_tax = $order_data['shipping_lines'][$key]["total_tax"];
                    $total_with_tax = $order_data['shipping_lines'][$key]["total"] + $order_data['shipping_lines'][$key]["total_tax"];
                    $order_data['shipping_lines'][$key]["total_wt_tax"] = $order_data['shipping_lines'][$key]["total"];
                    $order_data['shipping_lines'][$key]["total"] = $total_with_tax;
                }
            }
        }
        ...
        //
        $order_data['ysg_coupon_lines'] = $order_data['coupon_lines'];
...
function ysgSetDiscountType($order_item, $points, $coupon, $order_item_discount, $totals)
{
    ...
    if ($order_item_discount != false) {
        $p =  ysgGetCouponDiscount($order_item_discount, $order_item, $totals);
        //$all_discount_details[] = $p = ysgGetNormalDiscount($order_item_discount, $order_item, $totals);
        $p['type'] = "Discount";
        $all_discount_details[] = $p;
...
{
    //use only discount without tax
    $coupon_discount_total_with_vat = $coupon['discount'];

    //$coupon_discount_total_with_vat = $coupon['discount'] + $coupon['discount_tax'];

    $order_item_value = ($order_item['subtotal'] / $totals['amount']) * $coupon_discount_total_with_vat;
    $single_order_item_value = (($order_item['subtotal'] / $totals['amount']) / $order_item['quantity']) * $coupon_discount_total_with_vat;
    $a = array();
    ...
    $a['id'] = $coupon['id'];
    $a['type'] = "Coupon";
    $a['discount_total_no_tax'] = $coupon['discount'];
    $a['discount_tax'] = $coupon['discount_tax'];
    $a['type'] = "Coupon";
    $a['quantity'] = $order_item['quantity'];
...
function ysgGetCouponUsed($order)
{
    ...
    if (isset($order['meta_data']) && !empty($order['meta_data'])) {
        foreach ($order['meta_data'] as $key => $item) {
            $o = $item->get_data();
            if ($o['key'] == "ysg_coupon_type_name") {
                $coupon_value = strtolower($o['value']);
            }
            if ($o['key'] == "ysg_coupon_type" && strtolower($o['value']) == "coupon") {
                $coupon_name = strtolower($o['value']);
            }
        }
    }
    //ysgPrintParams($coupon_value);
    //ysgPrintParams($coupon_name);

    $coupon = FALSE;
    if (!empty($coupon_name)) {
        foreach ($order['coupon_lines'] as $key => $item) {
            if ($coupon_value ==  strtolower($order['coupon_lines'][$key]['code'])) {
                $coupon = $order['coupon_lines'][$key];
                break;
            }
        }
    }
    //ysgPrintParams($coupon);
...
/**
 * Get  coupon in the cart
 * @funciton ysgGetOrderDiscountUsed
 * @access public
 * @return array
 */
function ysgGetOrderDiscountUsed($order)
{
    $discount_name = "";
    $discount_value = "";
    ...
    if (isset($order['meta_data']) && !empty($order['meta_data'])) {
        foreach ($order['meta_data'] as $key => $item) {
            $o = $item->get_data();
            if ($o['key'] == "ysg_coupon_type_name") {
                $discount_value = strtolower($o['value']);
            }
            if ($o['key'] == "ysg_coupon_type" && strtolower($o['value']) == "discount") {
                $discount_name = strtolower($o['value']);
            }
        }
    }
    ...
    if (!empty($discount_name)) {
        foreach ($order['coupon_lines'] as $key => $item) {
            if ($discount_value ==  strtolower($order['coupon_lines'][$key]['code'])) {
                $coupon = $order['coupon_lines'][$key];
                break;
            }
        }
