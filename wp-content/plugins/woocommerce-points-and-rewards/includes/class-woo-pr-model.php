...
class Woo_Pr_Model
{
    ...
    public function woo_pr_get_user_checkout_points($cartdata, $user_id = 0, $order_id = 0)
    {
                        ...
                        if (!empty($pro_type) && $pro_type == 'woo_pr_points') {
                            ...
                            // Get points from product price
                                ...
                                if ($product) {
                                    ...
                                    $ysg_total = $ysg_total + $cart_item['line_total'];
        ...
        //YSG EDITS
        global $woocommerce;
        $ysg_dicount_applied = $this->woo_pr_get_discount_for_redeeming_points_from_cart($woocommerce->cart, true);

        /*
        echo "<br/>" . $ysg_dicount_applied; 
        echo "<br/>" . $ysg_total;
        echo "<br/>" . $total_points;
        ...
        $yg_discount_val = get_transient('woo_pr_add_discount_value_' . $user_id);
        $ysg_calc_big = !empty(get_option('woo_pr_ysg_ratio_multiplier')) ? get_option('woo_pr_ysg_ratio_multiplier') : 300;

        if ((isset($_POST['woo_pr_apply_discount']) && !empty($_POST['woo_pr_apply_discount']))
            || (!empty($yg_discount_val))
        ) {
            //discount is applied
            $ysg_total = $ysg_total - $ysg_dicount_applied;
            if ($ysg_total >= $ysg_calc_big) {
                $total_points = $ysg_total * 2;
            } else {
                $total_points = $ysg_total;
            }
        } else {
            if ($ysg_total >= $ysg_calc_big) {
                $total_points = $total_points * 2;
    ...
    public function woo_pr_get_discount_for_redeeming_points_from_cart($cart, $return_applied_discount = false)
    {
        ...
        $total_points =  $this->woo_pr_calculate_points($discount_applied);

        $new_points = ysgSubtractPendingOrderPoints($total_points, $userid);
        ...
        if (!empty($pro_type) && $pro_type == 'woo_pr_points') { //check product type is points 
            ...
            if (!empty($pro_type) && $pro_type == 'variable') { //check product type is varible
                ...
                if ($enable_tax_points == 'yes' && $product->is_taxable()) {
                    ...
                    //ysg edits
                    $woo_price = wc_get_price_excluding_tax($product);
                    //commented out
