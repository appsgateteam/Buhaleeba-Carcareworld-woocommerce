...
class Woo_Pr_Admin {
           ...
           while (($column = fgetcsv($file, 10000, ",")) !== FALSE) {
                ...
                {   
                 ...
                 if ( isset($userdata->ID) && !empty(($userdata->ID) && isset($csv_points) && !empty($csv_points) && $csv_points > 0))  { // Check user id and points are not empty
                        ...
                        $exportlog =  'Points were overrided by importing';
                        if($_POST['import_points_action'] != 'override_user_point'){
                            $exportlog = 'Points were added with existing points by importing';
                        }
                        ...
                        if($_POST['import_points_action'] != 'override_user_point'){
                          ...
                          $UserPointBalence = $ubalance+$csv_points;
                           update_user_meta($userdata->ID,'_woo_pr_userpoints',$UserPointBalence);  
                            ...
                            update_user_meta($userdata->ID,'_woo_pr_userpoints',$total_imp_point);  
                        }
    ...
    public function woo_pr_hide_save_changes_button_import_export_section(){
    ...
    function add_hooks() {
        ...
        //Hide Save changes button in import /export points setting section 
        add_action('init',array($this,'woo_pr_hide_save_changes_button_import_export_section'));
