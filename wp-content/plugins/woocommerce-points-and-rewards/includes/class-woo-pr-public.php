...
?>
                    <div class="woo-points-redeem-points-wrap">
                        <form method="POST" action="">
                            ...
                            <div class="woo-points-redeem-message">
        ...
        if (!empty($available_discount)) {
            ...
            $points_value       = apply_filters('woo_pr_cart_points_value', get_woocommerce_currency_symbol() . round($available_discount, 2), $available_discount);
        ...
        if (!empty($gotdiscount) && $is_automatic_redeem != 'yes') {
            ...
            <fieldset class="woo-pr-points-checkout-message">
                ...
                <div class="woo-pr-points-remove-disocunt-message"><?php printf(esc_html__('Remove %s Discount', 'woopoints'), $points_label); ?></div>
                <!--.woo-pr-points-checkout-message-->
            </fieldset>
            <!--.woo-pr-points-redeem-points-wrap-->
<?php
            $message_content = ob_get_clean();
        }
    ...
    function woo_pr_order_processing_completed_update_points($order_id)
    {
        ...
        $user_exclude_role = woo_pr_check_exclude_role($user_id);
        ...
        if (!empty($cart_data) && empty($old_points_order_earned) && woo_pr_check_min_cart_total_to_earn_points($order->get_subtotal()) && empty($earning_log)) {
            ...
            if ($totalpoints_earned  != 'user_product' && $totalpoints_earned != 0) {
                ...
                if ($is_enable_email == 'yes' && $is_enable_email_for_purchase_product == 'yes' && !empty($email_subject) && !empty($email_content)) { // If enable & not empty email subject & content			
                    ...
                    $total_amount_message  = sprintf(esc_html__("%s which are worth a amount of %s", "woopoints"), $total_point, wc_price(woo_pr_wcm_currency_convert($total_points_amount)));
    ...
    public function woo_pr_create_log_account_signup($user_id)
    {
        ...
        $user_exclude_role = woo_pr_check_exclude_role($user_id, true);
    ...
    public function woo_pr_woocommerce_checkout_process($order_id, $data)
    {
        ...
        $order_status = 'wc-' . $order_data['status'];
    ...
    public function woo_pr_rate_on_product($comment_ID, $comment)
    {
        ...
        if (
            !empty($woo_pr_enable_reviews) && ($woo_pr_enable_reviews == 'yes') && !empty($comment->user_id)
    ...
    public function woo_pr_rate_status_change($comment_ID, $comment_status)
    {
        ...
        if (
            ...
            // Check user role exclude
            $user_exclude_role = woo_pr_check_exclude_role($comment->user_id);
                ...
                foreach ($all_user_ids as $user_id) {
                    ...
                    if (!empty($today_points_data['add_points'])) {
                        ...
                        if ($remaining_points > 0) {
                            ...
                            $point_amount = sprintf('%s %s which are worth a amount of %s ', $remaining_points, $pointslable, $point_amount);
                            $site_url = '<a href="' . site_url() . '">' . site_url() . '</a>';
     ...
     * @since 1.0.0
     */
    function woo_pr_redeem_points_on_status_change($this_get_id,  $this_status_transition_from,  $this_status_transition_to,  $instance)
    {
