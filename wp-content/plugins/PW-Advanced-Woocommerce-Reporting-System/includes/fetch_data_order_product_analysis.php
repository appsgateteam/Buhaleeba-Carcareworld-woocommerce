<?php
	...
	if($file_used=="sql_table")
	{
		...
		$sql='';
		if ($pw_product_id!="-1")
			$products = $this->pw_get_simple_variation_product("SIMPLE",$pw_product_id,"ARRAY_A");
		else
			$products = $this->pw_get_simple_variation_product("SIMPLE",NULL,"ARRAY_A");
		...
		foreach($products  as $key=>$value):
            ...
            if($sql1==''):
                ...
                $sql_group_by= " GROUP By pw_qty.meta_value ";
		...
		$sql_columns= "
            ID as order_id, count(ID) as order_count,count(pw_qty.meta_value) as qty,  sum(pw_line_total.meta_value) as line_total,
            pw_order_items.order_item_name as order_item_name,
            pw_product_id.meta_value  as product_id";
		...
		$sql1 = "SELECT $sql_columns FROM $sql_joins  WHERE $sql_condition $pw_from_date_condition 	$sql_group_by";

		$sql_order_by='';
		if ($pw_sort_by == "quantity")
			$sql_order_by= " order By (qty) " . $pw_order_by;
		if ($pw_sort_by == "product_name")
			$sql_order_by= " order By order_item_name " . $pw_order_by;
