...
if (is_woocommerce_active()) {
	...
	include('admin/pw_cart_index_interface.php');
	include('admin/pw_cart_edit_interface.php');
    include('models/PW_Cart_Actions.php');
    include('models/PW_Cart_Receipt.php');
    include('helpers.php');
    ...
    */
    ...
    //Check first to see if we need to upgrade
        ...
        $check_sql = "SELECT meta_value FROM ".$wpdb->prefix. "postmeta WHERE meta_key = 'pw_cartitems'";
        ...
        if ($upgrade_needed) {
            //Upgrade needed.
            $check_sql = "SELECT * from ".$wpdb->prefix. "postmeta WHERE meta_key = 'pw_cartitems'";
	        echo $check_sql.'WWW';
	...
	woocommerce_abandoned_carts_activate();
    /**
    ...
    class PW_Cart_Reports
    {
        ...
        public function __construct()
        {
            ...
            $woocommerce_cart_reports_options['timeout'] = get_option(__PW_REPORT_WCREPORT_FIELDS_PERFIX__.'abandoned_cart_timeout');
            ...
            $pw_cart_reports_expiration_opt_in = get_option(__PW_REPORT_WCREPORT_FIELDS_PERFIX__.'abandoned_cart_expiration_opt_in');
            if ($pw_cart_reports_expiration_opt_in =='yes') {
                $woocommerce_cart_reports_options['pw_cart_reports_expiration_opt_in'] = true;
            } else {
                $woocommerce_cart_reports_options['pw_cart_reports_expiration_opt_in'] = false;
            }

            /* Cart Expiration Day range */

            $cart_expiration_opt_in = get_option(__PW_REPORT_WCREPORT_FIELDS_PERFIX__.'abandoned_cart_expiration_opt_in');
            $cart_expiration = get_option(__PW_REPORT_WCREPORT_FIELDS_PERFIX__.'abandoned_cart_expiration');
            ...
            if ($cart_expiration > 0 && $cart_expiration_opt_in != 'no') {
                ...
                $woocommerce_cart_reports_options['pw_cart_reports_expiration'] = $cart_expiration;
            } else {
                if (wp_next_scheduled('woocommerce_cart_reset')) {
                    $timestamp = wp_next_scheduled('woocommerce_cart_reset');
                    wp_unschedule_event($timestamp, 'woocommerce_cart_reset');
                }
                $woocommerce_cart_reports_options['pw_cart_reports_expiration'] = false;
            }
            ...
            $this->receipt = new PW_Cart_Receipt($session);
	        ...
	        if (is_admin()) {
		        $Edit_Interface = new PW_Edit_Interface();
		        $Cart_Index = new PW_Cart_Index_Page();
//		        $Settings_Page = new PW_Cart_Reports_Settings();
//		        $Dashboard_Page = new PW_Cart_Dashboard();
//		        $Reports = new PW_Cart_Reports_Page();
	        }
        ...
        {
            global $woocommerce_cart_reports_options;
            $expiration_days = $woocommerce_cart_reports_options['pw_cart_reports_expiration'];
            $opt_in_settings = $woocommerce_cart_reports_options['pw_cart_reports_expiration_opt_in'];
        ...
        public function save_receipt()
        {
	        ...
	        if (! cr_detect_search_engines($user_agent) && !is_restricted_role()) {
		        $receipt = new PW_Cart_Receipt($session);
     ...
     */
    function pw_woocommerce_cart_numbers($range = false)
    ...
    */
    ...
    $WooCommerce_Cart_Reports = new PW_Cart_Reports(); //Instantiate!!!
} //ENDCLASS

/**
 * Print out tool tip code, input contains desired text, requires
 */
function pw_tooltip($text, $print = true)
{
    global $woocommerce;
...
function is_restricted_role()
{
    ...
    $current_u = array_shift($user_roles); else:
        $current_u = 'guest';
    endif;
