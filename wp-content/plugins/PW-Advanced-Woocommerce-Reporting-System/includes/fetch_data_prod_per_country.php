...
//		$pw_to_date=substr($pw_to_date,0,strlen($pw_to_date)-3);
		...
		$sql_columns = "
			pw_woocommerce_order_itemmeta_product.meta_value 			as ids
		...
		if ($pw_from_date != NULL &&  $pw_to_date !=NULL)
    		$pw_from_date_condition = " AND DATE_FORMAT(shop_order.post_date, '%Y-%m-%d') BETWEEN ('" . $pw_from_date . "') and ('" . $pw_to_date . "')";
		...
		$sql = "SELECT $sql_columns 
				FROM $sql_joins $pw_region_code_join $category_id_join $brand_id_join $all_tax_joins 
				$pw_id_order_status_join 
				WHERE $sql_condition $pw_region_code_condition $pw_from_date_condition
				$category_id_condition $brand_id_condition $all_tax_conditions $pw_cat_prod_id_string_condition $pw_brand_prod_id_string_condition 
