...
if($file_used=="sql_table")
{
	...
	//CUSTOM WORK - 12300
	//print_r(get_option(__PW_REPORT_WCREPORT_FIELDS_PERFIX__.'tickera_custom_fields'));
	$meta=get_option(__PW_REPORT_WCREPORT_FIELDS_PERFIX__.'tickera_custom_fields');
	$tickera_cols='';
	$tickera_join='';
	$tickera_condition='';
	...
	$tickera_col_in=0;
	if(is_array($meta)){
		$tickera_join.=" left join {$wpdb->prefix}posts as tpost ON tpost.post_parent=pw_posts.ID ";
		$i=1;
		foreach($meta as $opt){
			...
			$post_tickera= $this->pw_get_woo_requests($label[1],NULL,true);
...
//		}
	...
	//CUSTOM WORK - 11899
	if ($pw_from_date_delivery != NULL &&  $pw_to_date_delivery !=NULL){
		$date_delivery_condition = " AND pw_delivery.meta_value  BETWEEN STR_TO_DATE('" . $pw_from_date_delivery . "', '$date_format') and STR_TO_DATE('" . $pw_to_date_delivery . "', '$date_format')";
	}
...
//print_r($ff_columns);
	...
	if($pw_detail_view=="yes"){
		...
		$columns=array(
			...
			//CUSTOM WORK - 12300
			array('lable'=>__('First Name',__PW_REPORT_WCREPORT_TEXTDOMAIN__),'status'=>'show'),
			array('lable'=>__('Last Name',__PW_REPORT_WCREPORT_TEXTDOMAIN__),'status'=>'show'),
			array('lable'=>__('Tickera Email',__PW_REPORT_WCREPORT_TEXTDOMAIN__),'status'=>'show'),

			//CUSTOM WORK - 11899
			//COMPATIBLE WITH ORDER DELIVERY PLUGIN
			array('lable'=>__('Delivery Date',__PW_REPORT_WCREPORT_TEXTDOMAIN__),'status'=>'show'),
			array('lable'=>__('Shipping Date',__PW_REPORT_WCREPORT_TEXTDOMAIN__),'status'=>'show'),
			...
			//CUSTOM WORK - 4187
			//array('lable'=>__('Trannsaction ID',__PW_REPORT_WCREPORT_TEXTDOMAIN__),'status'=>'show'),
		...
		if($pw_detail_view=="yes")
		{
			if($new_order)
			{
				...
				foreach($tickera_col as $col){
				    ...
				    $tickera_value=$this->pw_tickera_fetch_fields($items->order_id,$col);
...
//                    $datatable_value.=("</td>");
		...
		foreach($meta as $opt){
			...
			switch($field_type){
				...
				case "tc_radio_field_form_element":
					$_html='<select name="'.$field_html.'" id="'.$field_html.'"><option value="">'.__('Choose One',__PW_REPORT_WCREPORT_TEXTDOMAIN__).'</option>';
					if($field_content) {
    ...
    <form class='alldetails search_form_report' action='' method='post'>
        ...
        <div class="col-md-6">
            ...
            <span class="awr-form-icon"><i class="fa fa-calendar"></i></span>
            <input name="pw_from_date_delivery" id="pwr_from_date_delivery" type="text" readonly='true' class="datepick"/>
        </div>
        ...
        <div class="col-md-6">
            ...
            <span class="awr-form-icon"><i class="fa fa-calendar"></i></span>
            <input name="pw_to_date_delivery" id="pwr_to_date_delivery" type="text" readonly='true' class="datepick"/>
        </div>
        ...
        <div class="col-md-6">
            ...
            <span class="awr-form-icon"><i class="fa fa-calendar"></i></span>
            <input name="pw_id_order" type="text"  class="" placeholder="<?php _e('Separate IDs with (,) Example : 1,2',__PW_REPORT_WCREPORT_TEXTDOMAIN__);?>"/>
        </div>
		...
		if($tickera_html)
		{
            ...
            <div class="col-md-12 " >
                <div class="awr-form-title awr-product-addon-title" style="padding: 7px 5px 10px;text-align: center;background: #f2c811;color: #fff;">
					<?php _e('Tickera Custom Fields',__PW_REPORT_WCREPORT_TEXTDOMAIN__);?>
                </div>
