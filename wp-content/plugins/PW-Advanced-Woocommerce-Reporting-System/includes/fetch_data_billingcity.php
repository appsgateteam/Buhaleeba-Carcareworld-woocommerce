<?php
	...
	if($file_used=="sql_table")
	{
		...
		,pw_postmeta_city.meta_value AS 'billing_city_code'
		...
		LEFT JOIN  {$wpdb->prefix}postmeta as pw_postmeta_city ON pw_postmeta_city.post_id=pw_posts.ID
		...
		AND pw_postmeta_city.meta_key	=	'_billing_city'
		";
		...
		$sql_group_by = "  
		GROUP BY  pw_postmeta_city.meta_value";
		$sql_order_by="
		...
		//for($i=1; $i<=20 ; $i++){
			...
			$datatable_value.=("<tr>");
                ...
                $datatable_value.= $items->billing_city_code;
                $datatable_value.=("</td>");
