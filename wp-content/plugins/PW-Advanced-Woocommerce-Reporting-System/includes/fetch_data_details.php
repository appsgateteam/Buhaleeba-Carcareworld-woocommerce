...
//		}
	...
	$post_type_condition="pw_posts.post_type = 'shop_order' AND billing_country.meta_key	= '_billing_country' AND billing_state.meta_key	= '_billing_state'";
...
//print_r($ff_columns);
	...
	if($pw_detail_view=="yes"){
		...
		$columns=array(
			...
			array('lable'=>__('state',__PW_REPORT_WCREPORT_TEXTDOMAIN__),'status'=>'show'),
			...
			//CUSTOM WORK - 4187
			//array('lable'=>__('Transaction ID',__PW_REPORT_WCREPORT_TEXTDOMAIN__),'status'=>'show'),
        ...
        if(is_array(__CUSTOMWORK_ID__) && in_array('11899',__CUSTOMWORK_ID__)){
            ...
            $extra_cols[]=array('lable'=>__('Delivery Date',__PW_REPORT_WCREPORT_TEXTDOMAIN__),'status'=>'show');
			$extra_cols[]=array('lable'=>__('Shipping Date',__PW_REPORT_WCREPORT_TEXTDOMAIN__),'status'=>'show');
		...
		if(is_array(__CUSTOMWORK_ID__) && in_array('12679',__CUSTOMWORK_ID__)){
			$extra_cols=array();
			$extra_cols[]= array('lable'=>__('Clinic Type',__PW_REPORT_WCREPORT_TEXTDOMAIN__),'status'=>'show');
			array_splice($columns,4,0,$extra_cols);
		}
		...
		if(is_array(__CUSTOMWORK_ID__) && in_array('14957',__CUSTOMWORK_ID__)){
			$extra_cols=array();
			$extra_cols[]= array('lable'=>__('Transaction ID',__PW_REPORT_WCREPORT_TEXTDOMAIN__),'status'=>'show');
		...
		if(is_array(__CUSTOMWORK_ID__) && in_array('14957',__CUSTOMWORK_ID__)){
			$extra_cols=array();
			$extra_cols[]= array('lable'=>__('Childs Farm Order No',__PW_REPORT_WCREPORT_TEXTDOMAIN__),'status'=>'show');
		...
		if(is_array(__CUSTOMWORK_ID__) && in_array('19445',__CUSTOMWORK_ID__)){
			...
			foreach ( $easy_checkout_field as $key=>$field ){
				...
				if (strpos($key, 'billing_field') !== false) {
					...
					$extra_cols=array();
					$tlabel       = $field['label'];
					$extra_cols[] = array( 'lable'  => __( $tlabel, __PW_REPORT_WCREPORT_TEXTDOMAIN__ ),
					                       'status' => 'show'
					);
		...
		if(is_array(__CUSTOMWORK_ID__) && in_array('14957',__CUSTOMWORK_ID__)) {
			$extra_cols   = array();
			$extra_cols[] = array( 'lable'  => __( 'Childs Farm Order No', __PW_REPORT_WCREPORT_TEXTDOMAIN__ ),
			                       'status' => 'show'
			);
			array_splice( $columns, 1, 0, $extra_cols );
		}
	...
	foreach($this->results as $items){
		...
		if($pw_detail_view=="yes")
		{
			if($new_order)
			{
                ...
                //STATE
                $display_class='';
                $state      	= $this->pw_get_woo_bsn($items->billing_country,$items->billing_state);
