    ...
    if ( is_wp_error($user_check) ){
        ...
        if ( class_exists('WC_Vendors') && $_POST[ 'become_vendor' ] == 1 ) {
    		...
    		@wp_mail(get_option('admin_email'), sprintf(__('[%s] New User Registration', 'tz-feature-pack'), $blogname), $message);
				...
				$message  = sprintf(esc_html__('Welcome %s !', 'tz-feature-pack'), $info['user_login']) . "\r\n\r\n";
        $message .= sprintf(esc_html__("Thank you for creating your account at The Car Care World. ", 'tz-feature-pack')) . "\r\n\r\n";
        
        $message .= sprintf(esc_html__("To log-in to your account please visit ", 'tz-feature-pack'));
        $message .= wp_login_url() . "\r\n";
        $message .= sprintf(esc_html__('Username: %s', 'tz-feature-pack'), $info['user_login']) . "\r\n";
      // $message .= sprintf(esc_html__('Password: %s', 'tz-feature-pack'), $info['user_pass']) . "\r\n\r\n";
        $message .= sprintf(esc_html__('If you have any questions related to your account , email us at %s.', 'tz-feature-pack'), get_option('admin_email')) . "\r\n\r\n";
        $message .= esc_html__('This is an automatically generated email, please do not reply!', 'tz-feature-pack');

				if ( !$become_a_vendor ) {
				    

        	wp_mail($info['user_email'], sprintf(esc_html__('Welcome! Your The Car Care World Account Has Been Created!', 'tz-feature-pack'), $blogname), $message);
