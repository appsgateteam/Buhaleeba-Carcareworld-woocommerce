...
class Bookings {
	...
	public function schedule_booking_notification( $booking_id, $notification, $after_event = false ) {
		...
		if ( $booking ) {
			...
			if ( ! empty( $schedule ) ) {
				...
				$notification_timestamp = $after_event ? $notification_schedule->get_time_after( $booking->get_end() ) : $notification_schedule->get_time_before( $booking->get_start() );
	...
	public function send_booking_notification( $booking_id, $notification ) {
		...
		if ( $booking && $template = $this->get_template( $notification, $booking->get_product() ) ) {
			...
			if ( $this->is_admin_notification( $notification ) ) {
				...
				$phone_number = $booking->get_order()->get_billing_phone();
